...
class TreeVecVar:
    """
    this defines a vector variable for a tree, creating a single branch for it 
    the branch is vector of vectors 
       tree = tree to add branch to
       varnametype = name of branch and root type eg foo/float
       func = a unary callable object which acts on the object the branch is being filled from
    """
       
    def __init__(self,tree,varnametype,func):
        self.varname = varnametype.split("/")[0]
        self.vartype = varnametype.split("/")[1]
        self.func = func
        self.data = ROOT.std.vector(f"std::vector<{self.vartype}>")()
        self.create_branch(tree)

    def create_branch(self,tree):       
        tree.Branch(self.varname,self.data)
        
    def fill(self,obj,objnr):
        val = self.func(obj) if self.func else obj
        #objnr is 0 based
        if objnr >= self.data.size():
            self.data.resize(objnr+1)
        try:
            self.data[objnr] = val
        except (IndexError,TypeError) as err:
            #much easier in python3, small hack here for 2.7
