...
def process_dataset(dataset,xsec_data):
    ...
    files_txt = subprocess.Popen(['dasgoclient','--query','file dataset={}'.format(dataset),'--limit','-1'],stdout=subprocess.PIPE,universal_newlines=True).communicate()[0]
    files = files_txt.split("\n")
    file_name  = ",".join(files[4:8])
    cmssw_cfg = os.path.join(os.environ["CMSSW_BASE"],"src/Analysis/HLTAnalyserPy/test/getXSec_cfg.py")
    cmssw_cmd = ["cmsRun",cmssw_cfg,"inputFiles={}".format(file_name)]
    
    cmssw_log = subprocess.Popen(cmssw_cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,universal_newlines=True).communicate()[1]
    print(cmssw_log)
...
if __name__ == "__main__":
    ...
    query_results_txt = subprocess.Popen(['dasgoclient','--query',args.query,'--format','json'],stdout=subprocess.PIPE,universal_newlines=True).communicate()[0]
