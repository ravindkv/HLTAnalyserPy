...
class TSGRateTree:
    def __init__(self,tree_name,evtdata,trig_res_name="trig_res",default_l1_pass=True):   
        self.tree = ROOT.TTree(tree_name,'')
        self.evtdata = evtdata
        self.trig_res_name = trig_res_name
        self.initialised = False
        self.last_runnr = -1 #so we can watch for trigger menu changes
        self.default_l1_pass = default_l1_pass
 ...
 #       self.tree.Branch("hltRes",self.hlt_bits)
        ...
        if self.default_l1_pass:
            self.set_l1_alltrue()
        self.initialised = True

    def set_l1_alltrue(self):
        for bitnr in range(0,512):
            self.l1bits_initial.SetBitNumber(bitnr)
            self.l1bits_final.SetBitNumber(bitnr)
